...
**Update, 10-12-21** I realized I might need a file with all of the alien seqs in it, including the sponges. I think we are going to need to alter the /mnt/lustre/plachetzki/shared/metazoa_2020/above_80/OrthoFinder/Results_Oct19/Orthogroups/Orthogroups.tsv file in order to rerun the big R script that Sabrina wrote to analyze these data. Dave says Sabrina can handle it if we have a list of the sequences, so I'm going to make it the same way I did above. First, I copied the all_alien_seqs.txt file and created all_alien_seqs_with_sponges.txt. Then I did the same grep command as before, and tagged it onto the end of the new file, `grep -h -v "ID" sponge_indexing/*.blast.alien_index >> all_alien_seqs_with_sponges.txt`. This should hold all of the species that were not used in the Metazoa database, and are not outgroups.  
